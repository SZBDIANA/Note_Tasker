...
@Composable
fun terminadas(navController: NavHostController, tareasNotasViewModel: TareasNotasViewModel) {
    val tareasCompletadas by tareasNotasViewModel.tareasCompletadas.collectAsState(emptyList())
    var tareaSeleccionada by remember { mutableStateOf<Tarea?>(null) }
    var showDialog by remember { mutableStateOf(false) }

    LaunchedEffect(Unit) {
        tareasNotasViewModel.getTareasCompletadas()
    }
    ...
    Box(modifier = Modifier
        ...
        Column(
            ...
            encabezado(onClick = { navController.navigate("menu") }, R.drawable.menu_verde)
        }
        Column(modifier = Modifier.padding(vertical = 40.dp, horizontal = 20.dp)) {
            Text(stringResource(id = R.string.terminadas), fontSize = 30.sp, color = Color.Green)
